...
/-
  ...
  (2) Since we cannot eliminate all possible transports, we don't define a homomorphism as function
    M₁ i →lm M₂ (i + deg f)  or  M₁ i →lm M₂ (i + deg f)
...
--   (P : Π{i j} (p : i + deg f = j) (m : M₁ i) (n : M₂ j), Type)
--   (H : Πi m, P (right_inv (deg f) i) m (f ← i m)) {i j : I}
--   (p : i + deg f = j) (m : M₁ i) (n : M₂ j) : P p m (f ↘ p m) :=
-- begin
...
graded_hom.mk' (-d) (λi j p, fn j ∘lm homomorphism_of_eq (ap M₁ (eq_add_of_sub_eq p)))
...
definition graded_hom_mk_refl (d : I)
  (fn : Πi, M₁ i →lm M₂ (i + d)) {i : I} (m : M₁ i) : graded_hom.mk d fn i m = fn i m :=
by reflexivity

lemma graded_hom_mk_out'_destruct (d : I)
  (fn : Πi, M₁ (i + d) →lm M₂ i) {i : I} (m : M₁ (i + d)) :
  graded_hom.mk_out' d fn ↘ (add_sub_cancel i d) m = fn i m :=
begin
...
lemma graded_hom_mk_out_destruct (d : I)
  (fn : Πi, M₁ (i - d) →lm M₂ i) {i : I} (m : M₁ (i - d)) :
  graded_hom.mk_out d fn ↘ (sub_add_cancel i d) m = fn i m :=
begin
  unfold [graded_hom.mk_out],
  apply ap (λx, fn i (cast x m)),
  refine !ap_compose⁻¹ ⬝ ap02 _ _,
...
protected definition graded_iso.mk_out [constructor] (d : I) (φ : Πi, M₁ (i - d) ≃lm M₂ i) :
  M₁ ≃gm M₂ :=
begin
...
-- definition graded_homotopy.mk2 (hd : deg f ~ deg g) (hfn : Πi m, f i m =[hi + d] g i m) : f ~gm g :=
-- graded_homotopy.mk' hd
--   begin
--     intro i j pf pg m, induction (is_set.elim (hi + d ⬝ pg) pf), induction pg, esimp,
--     exact graded_hom_eq_transport f (hi + d) m ⬝ tr_eq_of_pathover (hfn i m),
--   end
...
-- definition graded_hom_compose_out {d₁ d₂ : I} (f₂ : Πi, M₂ i →lm M₃ (i + d₂))
--   (f₁ : Πi, M₁ (d₁⁻¹ i) →lm M₂ i) : graded_hom.mk d₂ f₂ ∘gm graded_hom.mk_out d₁ f₁ ~gm
--   graded_hom.mk_out_in d₁⁻¹ᵉ d₂ _ :=
-- _

-- definition graded_hom_out_in_compose_out {d₁ d₂ d₃ : I} (f₂ : Πi, M₂ (i + d₂) →lm M₃ (d₃ i))
--   (f₁ : Πi, M₁ (d₁⁻¹ i) →lm M₂ i) : graded_hom.mk_out_in d₂ d₃ f₂ ∘gm graded_hom.mk_out d₁ f₁ ~gm
--   graded_hom.mk_out_in (d₂ ⬝e d₁⁻¹ᵉ) d₃ (λi, f₂ i ∘lm (f₁ (i + d₂))) :=
-- begin
--   apply graded_homotopy.mk, intro i m, exact sorry
-- end

-- definition graded_hom_out_in_rfl {d₁ d₂ : I} (f : Πi, M₁ i →lm M₂ (i + d₂))
--   (p : Πi, i + d₁ = i) :
...
-- definition graded_homotopy_of_deg (d : I) (f g : graded_hom_of_deg d M₁ M₂) : Type :=
-- Π⦃i j : I⦄ (p : i + d = j), f p ~ g p

-- notation f ` ~[`:50 d:0 `] `:0 g:50 := graded_homotopy_of_deg d f g

-- variables {d : I} {f₁ f₂ : graded_hom_of_deg d M₁ M₂}

-- definition graded_homotopy_of_deg.mk [constructor] (h : Πi, f₁ (idpath (i + d)) ~ f₂ (idpath (i + d))) :
--   f₁ ~[d] f₂ :=
-- begin
--   intro i j p, induction p, exact h i
-- end

-- definition graded_homotopy.mk_out [constructor] {M₁ M₂ : graded_module R I} (d : I)
--   (fn : Πi, M₁ (i - d) →lm M₂ i) : M₁ →gm M₂ :=
